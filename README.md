...
112316110徐語楨心得:
在這次期末專題中，我主要負責main.py與analyzer.py的開發，工作內容包含系統主流程設計、人口統計分析、分析結果輸出，以及各模組之間的整合。
在main.py的部分，我負責呼叫各個模組，將原始資料讀入後進行清理，再計算各縣市人口總數、老化指數、男女比例、65歲以上人口男女比例，以及110至114年的人口變化趨勢。分析完成後，程式會將結果輸出成CSV檔案與圖表，並統一存放在results資料夾中。
在analyzer.py的部分，我使用Pandas與NumPy完成各項人口統計分析。計算人口總數時只篩選性別為「計」的資料，避免男女資料重複加總；計算老化指數時，使用65歲以上人口除以0至14歲人口，再乘以百分比。
在整合程式的過程中讓我更加理解模組化設計與團隊合作的重要性，也學會透過終端機輸出、CSV檔案與圖表互相比對，檢查分析結果是否合理，並且讓我更熟悉Python、Pandas、NumPy與模組化程式設計的能力。

113106102陳元隆心得:
這段時間做這個期末final專案，主要是透過Github雲端，能看到以往修改的內容，但組員間同時開發可能產生上傳問題跟利用Python做模組化開發，並處理政府110到114年的人口統計原始資料，剛開始拿到那堆密密麻麻、欄位分散的CSV檔案時，在資料清洗與欄位整合上花了不少時間，要跟ai溝通如何精準地將數據依照五齡組和性別進行處理。整個專案的核心在於數據的視覺化呈現例如基本的縣市人口長條圖與高齡人口男女比圓餅圖之外，協作過程中也遇到一些實際的工程問題，像是換自己電腦執行時，因為缺少Pandas或 Matplotlib套件而觸發 ModuleNotFoundError，或是因為VS Code工作目錄位置錯誤導致程式抓不到data資料夾，後來透過調整 Git 追蹤策略（像是利用 .gitignore 控管原始大檔案，但維持成果照片同步）以及微調路徑。最後順利將所有洗乾淨的CSV與自動生成的圖表同步到GitHub雲端，完成了整個從資料清理、指標分析到圖表產出的完整流程。
---
