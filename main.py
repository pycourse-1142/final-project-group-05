...
def main():
    ...
    print("計算人口成長變化...")
    change_df = calculate_population_change(
        trend_df,
        start_year=110,
        end_year=114
    )
    ...
    print("輸出分析結果 CSV...")
    population_114.to_csv(
        os.path.join(results_dir, "114_city_population.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    aging_index_114.to_csv(
        os.path.join(results_dir, "114_aging_index.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    gender_ratio_114.to_csv(
        os.path.join(results_dir, "114_gender_ratio.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    trend_df.to_csv(
        os.path.join(results_dir, "population_trend.csv"),
        index=False,
        encoding="utf-8-sig"
    )

    change_df.to_csv(
        os.path.join(results_dir, "population_change.csv"),
        index=False,
        encoding="utf-8-sig"
    )
    ...
    plot_population_bar(
        population_114,
        os.path.join(results_dir, "population_bar.png")
    )
...
elderly_gender_114,
    ...
    plot_population_trend(
        trend_df,
        os.path.join(results_dir, "population_trend.png")
    )
    ...
    print(
        aging_index_114
        .sort_values("aging_index", ascending=False)
        .head(10)
        .to_string(index=False)
    )
